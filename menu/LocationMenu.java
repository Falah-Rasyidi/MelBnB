...
public class LocationMenu extends Menu {
    ...
    public LocationMenu(PropertyDatabase propertyDatabase) {
        super(propertyDatabase);
    }
    
    public void initialise() {
        // Reset search builder and specify that we're be searching by location
        searchBuilder.setLength(0);
        searchBuilder.append("LOCATION,");

        String input = getUserSearch();

        while (input.strip().length() == 0) {
            System.out.println("Please input a location\n");
            input = getUserSearch();
        }
